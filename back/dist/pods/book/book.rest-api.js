...
var _book2 = require("./book.helpers");
...
booksApi.get('/', (0, _security.authorizationMiddleware)(), async (req, res, next) => {
  try {
    ...
    const bookList = await _dals.bookRepository.getBookList();
    const paginatedBookList = (0, _book2.paginateBookList)(bookList, page, pageSize);
    res.send((0, _book.mapBookListFromModelToApi)(paginatedBookList));
  } catch (error) {
    next(error);
  }
  ...
  try {
    const modelBook = (0, _book.mapBookFromApiToModel)(req.body);
    const newBook = await _dals.bookRepository.saveBook(modelBook);
  ...
  try {
    ...
    await _dals.bookRepository.saveBook(modelBook);
    res.sendStatus(204);
  } catch (error) {
    next(error);
  }
